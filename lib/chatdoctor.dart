...
class _chatDoctorState extends State<chatDoctor> {
  ...
  Future<void> _authenticateAndFetchDoctors() async {
    try {
      ...
      await _fetchDoctorsBasedOnAppointments();
    } catch (e) {
      setState(() {
  ...
  Future<void> _fetchDoctorsBasedOnAppointments() async {
    try {
      ...
      List<String> doctorIds = appointmentSnapshot.docs
      ...
      List<Doctor> doctors = doctorSnapshot.docs.map((doc) {
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        ChatScreen(userName: "fahad", receiverId: doctor.id),
                  ),
                );
              },
